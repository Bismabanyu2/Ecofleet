...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  // true untuk TPA, false untuk BBM
  bool _isTpaView = true;
  String _searchQuery = '';

  // Fungsi untuk mengekstrak nama sopir dari string detail
  String _getDriverName(ActivityModel item) {
    ...
    } else if (item.detail.contains('Op:')) {
      final parts = item.detail.split('Op:');
      if (parts.length > 1) {
        return parts[1].trim();
      }
    }
    return appData.nama; // Fallback jika tidak ditemukan
  }

  // Fungsi untuk menghapus data secara permanen dari appData.activities
  void _deleteActivity(ActivityModel targetItem) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Apakah Anda yakin ingin menghapus data aktivitas ini?', style: TextStyle(color: Colors.white70)),
        actions: [
          TextButton(
  ...
  // Fungsi untuk melihat detail & bukti foto (Ikon Mata)
  void _showDetailDialog(BuildContext context, ActivityModel item) {
    String driverName = _getDriverName(item);

    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        backgroundColor: const Color(0xFF1E293B),
        title: Text(item.title, style: const TextStyle(color: Color(0xFF22C55E), fontWeight: FontWeight.bold)),
        content: SizedBox(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                Text('Nama Sopir/Petugas: $driverName', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14)),
                const SizedBox(height: 8),
                Text('Waktu: ${item.time}', style: const TextStyle(color: Colors.white70, fontSize: 13)),
                const SizedBox(height: 8),
                Text('No. Truk: ${item.nomorKendaraan.isNotEmpty ? item.nomorKendaraan : "-"}', style: const TextStyle(color: Colors.white, fontSize: 13)),
                const SizedBox(height: 8),
                Text('Detail: ${item.detail}', style: const TextStyle(color: Colors.white70, fontSize: 13)),
                const SizedBox(height: 12),
                const Text('Bukti Foto Hasil:', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 12)),
                      ...
                      ? Image.memory(
                          ...
                          errorBuilder: (context, error, stackTrace) {
                            return Container(
                              ...
                              child: const Text('Gagal memuat gambar bukti.', style: TextStyle(color: Colors.white54, fontSize: 12)),
                            );
                          },
                        )
                      : Container(
                          ...
                          color: Colors.black26,
                          alignment: Alignment.center,
                          child: const Text('Tidak ada gambar bukti yang di-upload.', style: TextStyle(color: Colors.white54, fontSize: 12)),
                        ),
                ),
              ],
            ),
  ...
  // Fungsi Ekspor Semua Data Lengkap Beserta Bukti Foto ke PDF
  Future<void> _exportToPdf(BuildContext context, List<ActivityModel> list, String kategori) async {
    final pdf = pw.Document();

    // Mempersiapkan baris data tabel PDF secara asinkron agar gambar dimuat dengan benar
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (pw.Context context) => [
          pw.Header(
            level: 0,
            child: pw.Text('Laporan Operasional EcoFleet - $kategori',
                style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold)),
          ),
          pw.SizedBox(height: 10),
          pw.Table.fromTextArray(
            headers: ['No', 'Nama Sopir', 'Waktu', 'No Kendaraan', 'Detail Informasi', 'Bukti Foto'],
            data: tableData,
    ...
    // Cetak / Simpan File PDF otomatis ke perangkat
  ...
  Widget build(BuildContext context) {
    // Memisahkan data kegiatan berdasarkan TPA dan BBM
    ...
    final filteredList = currentList.where((item) {
      ...
      return item.detail.toLowerCase().contains(_searchQuery.toLowerCase()) ||
          ...
          driverName.toLowerCase().contains(_searchQuery.toLowerCase());
    }).toList();
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            tooltip: 'Ekspor PDF',
            onPressed: () => _exportToPdf(context, currentList, _isTpaView ? 'Data_TPA' : 'Data_BBM'),
          ),
          IconButton(
          ...
          Padding(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: 'Cari nomor truk, sopir, atau detail...',
            ...
            child: filteredList.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final driverName = _getDriverName(item);
                          ...
                          children: [
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  Text(
                                    '${item.nomorKendaraan.isNotEmpty ? item.nomorKendaraan : "Tanpa Plat"} • Sopir: $driverName',
